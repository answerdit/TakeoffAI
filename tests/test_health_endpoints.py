...
def test_post_verify_run_returns_summary(client):
    ...
    with patch(
        "backend.api.verification.run_verification_batch", new=AsyncMock(return_value=mock_result)
    ):
...
def test_post_verify_run_returns_500_on_error(client):
    with patch(
        "backend.api.verification.run_verification_batch",
...
def test_exclude_agent_adds_to_profile(tmp_path, monkeypatch):
    ...
    with TestClient(app) as c:
        resp = c.post(
            "/api/client/client1/exclude-agent",
            json={"agent_name": "aggressive"},
...
def test_reset_agent_history_clears_deviation(tmp_path, monkeypatch):
    ...
    with TestClient(app) as c:
        resp = c.delete(
            "/api/client/client2/agent-history/aggressive", headers={"X-API-Key": "test-key"}
        )
